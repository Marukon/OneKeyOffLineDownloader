    ...
    function writeDownList() {
        $.getJSON('./downlist',null,function (data) {
            ...
            for (var i = 0 ; i < data.length ; i++) {
                downlist.innerHTML = downlist.innerHTML + '<tr><td>' + i + '</td><td><a href="./data/gitdownload/' + data[i]['name'] + '">' + data[i]['name'] + '</a></td><td>' + data[i]['mtime'] + '</td><td>' + data[i]['size'] + '</td><td><button type="button" class="am-btn" onclick="del(\'' + data[i]['name'] + '\');">删除</button></td></tr>';
            }
        });
    }
    function download() {
